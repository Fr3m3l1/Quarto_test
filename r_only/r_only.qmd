...
reactive_taxonomic_levels <- reactive({
    if (is.null(input$otu_file)) {
        return(NULL) # or return(character(0)) - empty character vector
    }
    req(input$otu_file) # Now req is inside the if block
...
reactive_metadata_cols <- reactive({
    ...
    req(input$metadata_file) # Now req is inside the if block
