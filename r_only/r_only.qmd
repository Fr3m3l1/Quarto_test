...
observeEvent(uploaded_otu_table(), {
    output$download_report <- downloadHandler(
        filename = function() {
            paste0("microbiome-report-", Sys.Date(), ".pdf") # Changed filename
        },
        content = function(file) {
            temp_report <- tempfile(fileext = ".Rmd")

            # Rmd-Content Template
            report_content <- paste(
                "---",
                "title: 'Microbiome Analysis Report'",
                "output: pdf_document",
                "params:",
                "  data: NA", # Declared parameter 'data'
                "  metadata: NA", # Declared parameter 'metadata'
                "  tax_level: 'Genus'", # Declared parameter 'tax_level'
                "  metadata_group: 'Timepoint'", # Declared parameter 'metadata_group'
                "  alpha_indices: c('Shannon')", # Declared parameter 'alpha_indices'
                "---",
                "",
                ...
                "library(reshape2)",
                "library(gridExtra)",
                "otu_data <- params$data", # Get OTU data from params
                "metadata_data <- params$metadata", # Get metadata from params
                "tax_level <- params$tax_level",
                "metadata_group <- params$metadata_group",
                "alpha_indices <- params$alpha_indices",
                "count_cols <- names(otu_data)[grepl('^[AB]\\\\d{4}$', names(otu_data))]", # Assuming count columns start with A or B and 4 digits
                "```",
                "",
                ...
                "  select(SampleID, metadata_group, alpha_indices) %>%",
                ...
                "plots <- lapply(alpha_indices, function(index) {", # Loop through selected indices
                ...
                "})",
                "do.call(grid.arrange, c(plots, ncol = 1))", # Arrange plots in a column
                "```",
                "",
                "## Data Summary (First Few Rows of OTU Table)",
                "",
                "```{r data_summary_table_pdf, echo=FALSE}",
                "knitr::kable(head(otu_data))", # Display first few rows of uploaded OTU table in PDF
                "```",
                sep = "\n"
            )
            ...
            rmarkdown::render(
                ...
                output_file = file,
                params = list(
                    data = isolate(uploaded_otu_table()), # Pass uploaded OTU data
                    metadata = isolate(uploaded_metadata()), # Pass uploaded metadata
                    tax_level = isolate(input$tax_level), # Pass selected tax level
                    metadata_group = isolate(input$metadata_group), # Pass selected metadata group
                    alpha_indices = isolate(input$alpha_indices) # Pass selected alpha indices
                ),
...
renderDataTable({
    ...
    otu_table_display <- otu_data %>%
        select(SampleID = ID, input$tax_level, everything())
